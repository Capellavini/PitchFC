...
- **Design tokens:** all colors live in the `C` object (see `src/PitchApp.jsx`). Never hardcode a hex value in a component. Key tokens: `bg #07070A`, `card #18181E`, `accent #C8FF00` (acid lime), `green #00D08A` (success/paid), `orange #FF9F0A` (warning/pending), `red #FF3B5C` (declined).
- **Visual language:** dark theme only. 16px border-radius cards, 1px borders (`#22222C`), initials-based avatars with per-player palette colors, big 900-weight numbers for stats.
