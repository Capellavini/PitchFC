    ...
    return (
      <div style={{ ...cardStyle, marginBottom: 14 }}>
        ...
        <div style={{ fontSize: 11, color: C.text2, marginBottom: 12 }}>
          {list.length >= 2 ? "Escolhe o formato e começa a marcar os jogos." : "Sorteia as equipas para começar."}
        </div>
        ...
        <div style={{ fontSize: 11, color: C.text3, marginBottom: 14 }}>{MODES.find((m) => m.id === mode).hint}</div>
        ...
        {matchday.matches.map((m) => {
          ...
          return (
            <div key={m.id} style={{ background: C.surface, borderRadius: 14, padding: 14 }}>
              <div style={{ fontSize: 10, fontWeight: 800, letterSpacing: "0.1em", color: C.text3, marginBottom: 8 }}>JOGO {m.n}</div>
              ...
              {isPending ? (
                <div style={{ background: C.card, borderRadius: 12, padding: 12 }}>
                  <div style={{ fontSize: 11, color: C.text2, marginBottom: 8 }}>
                    {pending.scorerId
                      ? "Assistência de…"
                      : <>Golo dos <strong style={{ color: teamColor(pending.teamId) }}>{teamName(pending.teamId)}</strong> — quem marcou?</>}
                  </div>
                  <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                    ...
                    <button onClick={() => setPending(null)} style={{ background: "none", color: C.text3, border: "none", fontSize: 12, cursor: "pointer" }}>Cancelar</button>
                  </div>
                </div>
              ) : (
      ...
      {composing && (
        <div style={{ background: C.surface, borderRadius: 12, padding: 12, marginBottom: 12 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
            <span style={{ fontSize: 12, fontWeight: 700 }}>Quem joga agora?</span>
          ...
          {composing.homeId === composing.awayId && <div style={{ fontSize: 11, color: C.red, marginTop: 8 }}>Escolhe duas equipas diferentes.</div>}
